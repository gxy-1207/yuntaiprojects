...
public interface MessageService {
    ...
    /**
     * 同步消息到服务器 uid 是 string 用户id lsid 是 string 律师id content 是 string 发送内容（用urlencode加密） type 是
     * string 1 用户发送 2 律师发送
     */
    @POST("Chat/addMsgs")
    Observable<AddMessageResponseBean> syncMessageToService(
    ...
    Observable<HistoryRecordResponseBean> getHistoryRecord(
            @Query("uid") String uid, @Query("lsid") String lsid, @Query("source") int source);
    /** 获取聊天消息历史记录 */
    @GET("Newzixun/get_chat_news")
    Observable<HistoryChatResponse> userGetHistoryRecord(
            @Query("uid") String uid, @Query("lsid") String lsid, @Query("source") int source);

    /** 获取聊天消息历史记录 */
    @GET("Newzixun/get_chat_news")
    Observable<HistoryChatResponse> lawyerGetHistoryRecord(
            @Query("uid") String uid, @Query("lsid") String lsid, @Query("source") int source);
}
