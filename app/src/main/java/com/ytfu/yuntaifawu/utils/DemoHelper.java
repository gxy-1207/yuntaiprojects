...
import com.ytfu.yuntaifawu.db.DemoDBManager;
import com.ytfu.yuntaifawu.db.InviteMessgeDao;
import com.ytfu.yuntaifawu.db.LvShiDao;
import com.ytfu.yuntaifawu.db.UserDao;
import com.ytfu.yuntaifawu.domain.InviteMessage;
import com.ytfu.yuntaifawu.domain.InviteMessage.InviteMessageStatus;
import com.ytfu.yuntaifawu.domain.RobotUser;
import com.ytfu.yuntaifawu.parse.UserProfileManager;
import com.ytfu.yuntaifawu.receiver.CallReceiver;
import com.ytfu.yuntaifawu.receiver.HeadsetReceiver;
...
public class DemoHelper {
    ...
    private EMOptions initChatOptions(Context context) {
        ...
        builder.enableVivoPush() // 需要在AndroidManifest.xml中配置appId和appKey
                ...
                .enableOppoPush(
                        "65872dc4c26a446a8f29014f758c8272", "9385ae4308d64b36bf82bc4d73c4369d")
        ...
        if (demoModel.isCustomServerEnable()
                && demoModel.getRestServer() != null
                && demoModel.getIMServer() != null) {
        ...
        if (demoModel.isCustomAppkeyEnabled()
                && demoModel.getCutomAppkey() != null
                && !demoModel.getCutomAppkey().isEmpty()) {
    ...
    private void setCallOptions() {
        ...
        if (maxFrameRate != -1) {
            EMClient.getInstance()
                    .callManager()
                    .getCallOptions()
                    .setMaxVideoFrameRate(maxFrameRate);
        }
        ...
        /**
         * This function is only meaningful when your app need recording If not, remove it. This
         * function need be called before the video stream started, so we set it in onCreate
         * function. This method will set the preferred video record encoding codec. Using default
         * encoding format, recorded file may not be played by mobile player.
         */
        ...
        if (wh.length == 2) {
            try {
                EMClient.getInstance()
                        ...
                        .getCallOptions()
                        .setVideoResolution(
                                new Integer(wh[0]).intValue(), new Integer(wh[1]).intValue());
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
        ...
        EMClient.getInstance()
                .callManager()
                .getCallOptions()
                .enableFixedVideoResolution(enableFixSampleRate);

        // Offline call push
        EMClient.getInstance()
                .callManager()
                .getCallOptions()
                .setIsSendPushIfOffline(getModel().isPushCall());
        ...
        EMClient.getInstance()
                .callManager()
                .getCallOptions()
                .setExternalAudioParam(isExternalAudio, hz, 1);
    }
    ...
    protected void setEaseUIProviders() {
        ...
        // set options
        easeUI.setSettingsProvider(
                new EaseSettingsProvider() {
                ...
                .setNotificationInfoProvider(
                        new EaseNotificationInfoProvider() {
                            ...
                            public String getDisplayedText(EMMessage message) {
                                // be used on notification bar, different text according the message
                                // type.
                                String ticker =
                                        EaseCommonUtils.getMessageDigest(message, appContext);
                                ...
                                if (user != null) {
                                    if (EaseAtMessageHelper.get().isAtMeMsg(message)) {
                                        return String.format(
                                                appContext.getString(R.string.at_your_in_group),
                                                user.getNickname());
                                    }
                            ...
                            public String getLatestText(
                                    EMMessage message, int fromUsersNum, int messageNum) {
                                // here you can customize the text.
                                // return fromUsersNum + "contacts send " + messageNum + "messages
                                // to you";
                                return null;
                            }
                            ...
                            public Intent getLaunchIntent(EMMessage message) {
                                Log.e("TAG", "-------------->getLaunchIntent");
                                // you can set what activity you want display when user click the
                                // notification

                                String type =
                                        SpUtil.getString(appContext, AppConstant.SHENFEN, "1");
                                if ("1".equals(type)) {
                                    String toUserId = message.getFrom();
                                    ConversationBean.MsgBean msgBean =
                                            LvShiDao.getInstance(appContext).selectById(toUserId);
                                    String toUserName = msgBean.getName();
                                    String toUserAvatar = msgBean.getPicurl();
                                    return UserChatRoomActivity.getStartIntent(
                                            appContext,
                                            false,
                                            true,
                                            toUserId,
                                    ...
                                    String toUserName = msgBean.getName();
                                    String toUserAvatar = msgBean.getPicurl();
                                    return LawyerChatRoomActivity.getStartIntent(
                                            appContext,
                                            true,
                                            toUserId,
                                            toUserName,
                                ...
                                //                    if (chatType == ChatType.Chat) { // single
                                // chat message
                ...
                new EMConnectionListener() {
                    ...
                    public void onDisconnected(int error) {
                        ...
                        if (error == EMError.USER_REMOVED) {
                            onUserException(AppConstant.ACCOUNT_REMOVED);
                        } else if (error == EMError.USER_LOGIN_ANOTHER_DEVICE) {
                            onUserException(AppConstant.ACCOUNT_CONFLICT);
                        } else if (error == EMError.SERVER_SERVICE_RESTRICTED) {
                            onUserException(AppConstant.ACCOUNT_FORBIDDEN);
                        } else if (error == EMError.USER_KICKED_BY_CHANGE_PASSWORD) {
                            onUserException(AppConstant.ACCOUNT_KICKED_BY_CHANGE_PASSWORD);
                        } else if (error == EMError.USER_KICKED_BY_OTHER_DEVICE) {
                            onUserException(AppConstant.ACCOUNT_KICKED_BY_OTHER_DEVICE);
                        }
                    }
                        ...
                        new EMConferenceListener() {
                            ...
                            public void onMemberJoined(EMConferenceMember member) {
                                EMLog.i(
                                        TAG,
                                        String.format(
                                                "member joined username: %s, member: %d",
                                                member.memberName,
                                                EMClient.getInstance()
                                                        .conferenceManager()
                            ...
                            public void onMemberExited(EMConferenceMember member) {
                                EMLog.i(
                                        TAG,
                                        String.format(
                                                "member exited username: %s, member size: %d",
                                                member.memberName,
                                                EMClient.getInstance()
                                                        .conferenceManager()
                                                        .getConferenceMemberList()
                            ...
                            public void onStreamAdded(EMConferenceStream stream) {
                                EMLog.i(
                                        TAG,
                                        String.format(
                                                "Stream added streamId: %s, streamName: %s, memberName: %s, username: %s, extension: %s, videoOff: %b, mute: %b",
                                                stream.getStreamId(),
                                                stream.getStreamName(),
                                                stream.getMemberName(),
                                                stream.getUsername(),
                                                stream.getExtension(),
                                                stream.isVideoOff(),
                                                stream.isAudioOff()));
                                EMLog.i(
                                        TAG,
                                        String.format(
                                                "Conference stream subscribable: %d, subscribed: %d",
                                                EMClient.getInstance()
                                                        .conferenceManager()
                            ...
                            public void onStreamRemoved(EMConferenceStream stream) {
                                EMLog.i(
                                        TAG,
                                        String.format(
                                                "Stream removed streamId: %s, streamName: %s, memberName: %s, username: %s, extension: %s, videoOff: %b, mute: %b",
                                                stream.getStreamId(),
                                                stream.getStreamName(),
                                                stream.getMemberName(),
                                                stream.getUsername(),
                            ...
                            public void onPassiveLeave(int error, String message) {
                                EMLog.i(
                                        TAG,
                                        String.format(
                                                "passive leave code: %d, message: %s",
                                                error, message));
                            }
                            ...
                            public void onReceiveInvite(
                                    String confId, String password, String extension) {
                                EMLog.i(
                                        TAG,
                                        String.format(
                                                "Receive conference invite confId: %s, password: %s, extension: %s",
    ...
    public void goConference(String confId, String password, String extension) {
        ...
        try {
            ...
            inviter = jsonObj.optString(AppConstant.EXTRA_CONFERENCE_INVITER);
            groupId = jsonObj.optString(AppConstant.EXTRA_CONFERENCE_GROUP_ID);
        } catch (JSONException e) {
            e.printStackTrace();
        }

        //        ConferenceActivity.receiveConferenceCall(appContext, confId, password, inviter,
        // groupId);
    }
    ...
    private boolean isDuringMediaCommunication() {
        ...
        if (easeUI.hasForegroundActivies()
                && ("LiveActivity".equals(topClassName)
                        || "ConferenceActivity".equals(topClassName))) {
            return true;
        }
        return false;
    }
    ...
    class MyGroupChangeListener implements EMGroupChangeListener {
        ...
        public void onInvitationReceived(
            ...
            broadcastManager.sendBroadcast(new Intent(AppConstant.ACTION_GROUP_CHANAGED));
        }
        ...
        public void onInvitationAccepted(String groupId, String invitee, String reason) {
            ...
            showToast(
                    invitee + "Accept to join the group：" + _group == null
                            ? groupId
                            : _group.getGroupName());
        ...
        @Override
        public void onRequestToJoinReceived(
                String groupId, String groupName, String applyer, String reason) {
        ...
        @Override
        public void onMuteListAdded(
                String groupId, final List<String> mutes, final long muteExpire) {
    ...
    public class MyContactListener implements EMContactListener {
        ...
        public void onContactAdded(String username) {
            ...
            broadcastManager.sendBroadcast(new Intent(AppConstant.ACTION_CONTACT_CHANAGED));
    ...
    public class MyMultiDeviceListener implements EMMultiDeviceListener {
        ...
        public void onContactEvent(int event, String target, String ext) {
            switch (event) {
                case EMMultiDeviceListener.CONTACT_REMOVE:
                    {
                        ...
                        broadcastManager.sendBroadcast(
                                new Intent(AppConstant.ACTION_CONTACT_CHANAGED));
                ...
                case EMMultiDeviceListener.CONTACT_ACCEPT:
                    {
                        ...
                        updateContactNotificationStatus(
                                target, "", InviteMessageStatus.MULTI_DEVICE_CONTACT_ACCEPT);
                        broadcastManager.sendBroadcast(
                                new Intent(AppConstant.ACTION_CONTACT_CHANAGED));
                ...
                case EMMultiDeviceListener.CONTACT_DECLINE:
                    updateContactNotificationStatus(
                            target, "", InviteMessageStatus.MULTI_DEVICE_CONTACT_DECLINE);
                ...
                case CONTACT_BAN:
                    updateContactNotificationStatus(
                            target, "", InviteMessageStatus.MULTI_DEVICE_CONTACT_BAN);
                ...
                case CONTACT_ALLOW:
                    updateContactNotificationStatus(
                            target, "", InviteMessageStatus.MULTI_DEVICE_CONTACT_ALLOW);
        ...
        private void updateContactNotificationStatus(
                String from, String reason, InviteMessageStatus status) {
        ...
        public void onGroupEvent(
                final int event, final String target, final List<String> usernames) {
            execute(
                    new Runnable() {
                        @Override
                        public void run() {
                            try {
                                ...
                                switch (event) {
                                    ...
                                    case GROUP_DESTROY:
                                        ...
                                        broadcastManager.sendBroadcast(
                                                new Intent(AppConstant.ACTION_GROUP_CHANAGED));
                                        break;
                                    case GROUP_JOIN:
                                    ...
                                    case GROUP_APPLY_ACCEPT:
                                        ...
                                        inviteMessgeDao.deleteGroupMessage(
                                                groupId, usernames.get(0));
                                        // TODO: person, reason from ext
                                        saveGroupNotification(
                                                groupId, /*groupName*/
                                                "", /*person*/
                                                usernames.get(0), /*reason*/
                                                "",
                                                InviteMessageStatus
                                    ...
                                    case GROUP_INVITE_ACCEPT:
                                        ...
                                        msg.setMsgId(UUID.randomUUID().toString());
                                        msg.addBody(
                                                new EMTextMessageBody(msg.getFrom() + " " + st3));
                                        msg.setStatus(EMMessage.Status.SUCCESS);
                                        // save invitation as messages
                                        EMClient.getInstance().chatManager().saveMessage(msg);
        ...
        private void saveGroupNotification(
                String groupId,
                String groupName,
                String inviter,
                String reason,
        ...
        private void updateGroupNotificationStatus(
                String groupId,
                String groupName,
                String inviter,
                String reason,
    ...
    protected void onUserException(String exception) {
        ...
        String type = SpUtil.getString(appContext, AppConstant.SHENFEN, "");
        if ("1".equals(type)) {
            UserMainActivity.Companion.start(appContext, 0);
        } else {
    ...
    /**
     * Global listener If this event already handled by an activity, you don't need handle it again
     * activityList.size() <= 0 means all activities already in background or not in
     * ActivityZhiYeJiGou Stack
     */
                ...
                new EMMessageListener() {
                    ...
                    public void onMessageReceived(List<EMMessage> messages) {
                        ...
                        for (EMMessage message : messages) {
                            ...
                            //                    String userName =
                            // message.getStringAttribute("hxid_nickname", "");
                            //                    String userPic =
                            // message.getStringAttribute("hxid_avatar", "");
                            ...
                            // 判断一下是否是会议邀请
                            String confId =
                                    message.getStringAttribute(AppConstant.MSG_ATTR_CONF_ID, "");
                            if (!"".equals(confId)) {
                                String password =
                                        message.getStringAttribute(
                                                AppConstant.MSG_ATTR_CONF_PASS, "");
                                String extension =
                                        message.getStringAttribute(
                                                AppConstant.MSG_ATTR_EXTENSION, "");
                                goConference(confId, password, extension);
                            }
                    ...
                    public void onMessageRecalled(List<EMMessage> messages) {
                        for (EMMessage msg : messages) {
                            if (msg.getChatType() == ChatType.GroupChat
                                    && EaseAtMessageHelper.get().isAtMeMsg(msg)) {
        ...
        new Thread() {
            ...
            public void run() {
                try {
                    ...
                    // in case that logout already before server returns, we should return
                    // immediately
                    if (!isLoggedIn()) {
                ...
                try {
                    ...
                    getUserProfileManager()
                            .asyncFetchContactInfosFromServer(
                                    usernames,
                                    new EMValueCallBack<List<EaseUser>>() {
