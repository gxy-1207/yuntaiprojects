...
*
*/
public class DataAdaper extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    private Context mContext;
    private List<AudioListBean.ListBean> mList;

    public DataAdaper(Context mContext) {
        this.mContext = mContext;
        mList = new ArrayList<>();
    }

    public void setList(List<AudioListBean.ListBean> list) {
        this.mList = list;
        notifyDataSetChanged();
    }

    public void addList(List<AudioListBean.ListBean> list) {
        if (!list.isEmpty()) {
            mList.addAll(list);
            notifyDataSetChanged();
        }
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        ...
        AudioListBean.ListBean listBean = mList.get(position);
//        Glide.with(mContext).load(listBean.getPost_img()).into(viewHolder.image);
        ...
        viewHolder.text_count.setText("已有" + listBean.getOrder_count() + "人咨询");
