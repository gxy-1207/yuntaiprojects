...
/** @Auther gxy @Date 2020/6/10 @Des 公告presenter */
public class AnnouncementPresenter
    extends BaseRefreshPresenter<BaseRefreshView<AnnouncementBean.DataBean>> {

  // 获取公告列表
  public void getAnnouncementList(boolean isLoadMore, int page, String uid) {
    AnnouncementService service = HttpUtil.getInstance().getService(AnnouncementService.class);
    Observable<AnnouncementBean> observable = service.setAnnouncementList(page, uid);
    HttpUtil.getInstance()
        .enqueue(
            observable,
            bindLifecycle(),
            new BaseRxObserver<AnnouncementBean>() {
              @Override
