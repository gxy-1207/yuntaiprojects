...
public class UserChatRoomPresenter extends BasePresenter<UserChatRoomView>
     ...
     */
    public void getFeePayOrder(
            boolean isWechatPay, String selfId, String toUserId, String redPackageId) {
        PayService service = HttpUtil.getInstance().getService(PayService.class);
        if (isWechatPay) {
            Observable<FeeWechatOrderResponse> ob =
                    service.getFeeWechatPayOrder(selfId, toUserId, redPackageId, 12, 1);
            HttpUtil.getInstance()
                    .enqueue(
                            ob,
                            bindLifecycle(),
                            new BaseRxObserver<FeeWechatOrderResponse>() {
                                ...
                                public void onNextImpl(
                                        FeeWechatOrderResponse feeWechatOrderResponse) {
            ...
            Observable<PayBean> ob =
                    service.getFeeAliPayOrder(selfId, toUserId, redPackageId, 12, 1);
            HttpUtil.getInstance()
                    .enqueue(
                            ob,
                            bindLifecycle(),
    ...
    public void getLawyerCardInfo(String lawyerId) {
        Observable<RoomLawyerCardInfoResponse> ob =
                HttpUtil.getInstance().getService(LawyerService.class).getLawyerCardInfo(lawyerId);
        HttpUtil.getInstance()
                .enqueue(
                        ob,
                        bindLifecycle(),
                        new BaseRxObserver<RoomLawyerCardInfoResponse>() {
                            ...
                            public void onNextImpl(RoomLawyerCardInfoResponse lvShiCardBean) {
                                ...
                                RoomLawyerCardInfoResponse.LawyerCardInfo data =
                                        lvShiCardBean.getData();
                                if (null == data) {
    ...
    public void getCustomerServiceMessage() {
        Observable<ZiXunSendMessageBean> ob =
                HttpUtil.getInstance().getApiService().setZiXunMessage();
        HttpUtil.getInstance()
                .enqueue(
                        ob,
                        bindLifecycle(),
    ...
    public void checkPay(int requestCode, String selfId, String toUserId) {
        Observable<WhetherToPayBean> ob =
                HttpUtil.getInstance().getService(PayService.class).checkPay(selfId, toUserId);
        HttpUtil.getInstance()
                .enqueue(
                        ob,
                        bindLifecycle(),
                        new BaseRxObserver<WhetherToPayBean>() {
                            @Override
    ...
    public void exchangeWeChat(String selfId, String toUserId) {
        ...
        Observable<ExChangeWeiXinBean> ob =
                HttpUtil.getInstance().getApiService().setExChangeWx(map);
        HttpUtil.getInstance()
                .enqueue(
                        ob,
                        bindLifecycle(),
                        new BaseRxObserver<ExChangeWeiXinBean>() {
                            @Override
                            public void onSubscribeImpl(Disposable d) {
                                super.onSubscribeImpl(d);
                                getView().showProgress();
    ...
    /** 律師同步消息到服務器 */
    public void userSyncMessageToService(
            String toUserId, String fromUserId, String content, String imageUrl) {
        HttpUtil.getInstance()
                .getService2(MessageService.class)
                .lawyerSyncMessageToService("", fromUserId, toUserId, content, "1", imageUrl)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .as(bindLifecycle())
                .subscribe(
    ...
    public void checkMessagePayed(String selfId, String toUserId, String feeId) {
        Observable<ToCheckPayBean> ob =
                createService(ApiService.class).setToPay(selfId, toUserId, feeId);
        requestRemote(
                ob,
                new BaseRxObserver<ToCheckPayBean>() {
                    @Override
                    public void onSubscribeImpl(Disposable d) {
                        super.onSubscribeImpl(d);
                        getView().showProgress();
    ...
    public void payFeeByAccount(String selfId, String toUserId, String redPackageId) {
        Observable<AccountPayResponseBean> ob =
                HttpUtil.getInstance()
                        .getService(PayService.class)
    ...
    public void getRefundBtnVisibe() {
        Observable<RefundButtonVisibleBean> observable =
                createService(RefundService.class).setRefundButtonVisible();
        requestRemote(
                observable,
                new BaseRxObserver<RefundButtonVisibleBean>() {
                    @Override
