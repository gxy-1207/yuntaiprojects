...
import com.hyphenate.easeui.widget.presenter.EaseChatPayPresenter;
import com.hyphenate.easeui.widget.presenter.EaseChatPayYfPresenter;
import com.hyphenate.easeui.widget.presenter.EaseChatRowCardPresenter;
...
public class EaseMessageAdapter extends BaseAdapter {
    ...
    private static final int MESSAGE_TYPE_SENT_PAY = 14;
    private static final int MESSAGE_TYPE_RECY_PAY = 15;
    private static final int MESSAGE_TYPE_SENT_PAY_YF = 16;
    private static final int MESSAGE_TYPE_RECY_PAY_YF = 17;
    private static final int MESSAGE_TYPE_SENT_CARD= 18;
    private static final int MESSAGE_TYPE_RECY_CARD = 19;
    ...
    public EaseMessageAdapter(Context context,String toUSerPic, String username, int chatType, ListView listView) {
    ...
    public int getItemViewType(int position) {
        ...
        if (message.getType() == EMMessage.Type.TXT) {
            ...
            } else if (("1").equals(message.getStringAttribute(EaseConstant.MESSAGE_PAY_TYPE, ""))) {//自定义消息类型
                //支付
                return message.direct() == EMMessage.Direct.RECEIVE ? MESSAGE_TYPE_RECY_PAY : MESSAGE_TYPE_SENT_PAY;
            } else if (("2").equals(message.getStringAttribute(EaseConstant.MESSAGE_PAY_TYPE, ""))) {
                //用户支付完成  律师回复
                return message.direct() == EMMessage.Direct.RECEIVE ? MESSAGE_TYPE_RECY_PAY_YF : MESSAGE_TYPE_SENT_PAY_YF;
            } else if (("4").equals(message.getStringAttribute("ctype", ""))) {
                return message.direct() == EMMessage.Direct.RECEIVE ? MESSAGE_TYPE_RECY_CARD : MESSAGE_TYPE_SENT_CARD;
            }
    ...
    protected EaseChatRowPresenter createChatRowPresenter(EMMessage message, int position) {
        ...
        switch (message.getType()) {
            case TXT:
                ...
                } else if (("1").equals(message.getStringAttribute(EaseConstant.MESSAGE_PAY_TYPE, ""))) {
                    presenter = new EaseChatPayPresenter();//需要自己创建
                } else if (("2").equals(message.getStringAttribute(EaseConstant.MESSAGE_PAY_TYPE, ""))) {
                    presenter = new EaseChatPayYfPresenter();
                } else if(("4").equals(message.getStringAttribute("ctype", ""))){
                   presenter = new EaseChatRowCardPresenter();
                }else {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            presenter.setToUserPic(toUSerPic);
